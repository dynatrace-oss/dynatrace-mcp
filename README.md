...
```bash
# Add Dynatrace MCP server with environment variables
gemini mcp add dynatrace npx @dynatrace-oss/dynatrace-mcp-server@latest \
  --env DT_PLATFORM_TOKEN=<platform-token> \
  --env DT_ENVIRONMENT=<dt-environment-url> \
  --timeout 30000
```

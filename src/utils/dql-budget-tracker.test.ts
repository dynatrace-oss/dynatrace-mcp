import {
  createDqlBudgetTracker,
  getDqlBudgetTracker,
  resetDqlBudgetTracker,
  formatBytesAsGB,
  generateBudgetWarning,
} from './dql-budget-tracker';

describe('DQL Budget Tracker', () => {
  beforeEach(() => {
    resetDqlBudgetTracker();
  });

  describe('createDqlBudgetTracker', () => {
    it('should create a tracker with the specified budget limit', () => {
      const tracker = createDqlBudgetTracker(5);
    ...
    it('should correctly track bytes scanned', () => {
      const tracker = createDqlBudgetTracker(1); // 1 GB limit
    ...
    it('should accumulate bytes scanned across multiple calls', () => {
      const tracker = createDqlBudgetTracker(2); // 2 GB limit
    ...
    it('should reset tracker correctly', () => {
      const tracker = createDqlBudgetTracker(1);
  ...
  describe('getDqlBudgetTracker', () => {
    ...
    it('should create a global tracker with specified budget when called first time', () => {
      const tracker = getDqlBudgetTracker(15);
      const state = tracker.getState();
    ...
    it('should return the same global tracker instance on subsequent calls', () => {
      const tracker1 = getDqlBudgetTracker(5);
      const tracker2 = getDqlBudgetTracker(20); // Different budget, but should return existing

      expect(tracker1).toBe(tracker2);
      expect(tracker1.getState().budgetLimitGB).toBe(5); // Should keep original budget
    });

    it('should persist state across getDqlBudgetTracker calls', () => {
      const tracker1 = getDqlBudgetTracker(3);
      tracker1.addBytesScanned(1000000000); // 1 GB

      const tracker2 = getDqlBudgetTracker();
  ...
  describe('generateBudgetWarning', () => {
    it('should return budget exceeded warning when budget is exceeded', () => {
      ...
      expect(warning).toContain('🚨 **DQL Budget Exceeded:**');
    ...
    it('should return warning when approaching budget (80% threshold)', () => {
      ...
      expect(warning).toContain('⚠️ **DQL Budget Warning:**');
    ...
    it('should prioritize exceeded warning over approaching warning', () => {
      ...
      expect(warning).toContain('🚨 **DQL Budget Exceeded:**');
      expect(warning).not.toContain('⚠️ **DQL Budget Warning:**');
    });
  });

  describe('edge cases', () => {
    it('should handle fractional GB budgets', () => {
      const tracker = createDqlBudgetTracker(0.5); // 0.5 GB limit
      const state = tracker.getState();
    ...
    it('should handle very large budgets', () => {
      const tracker = createDqlBudgetTracker(1000); // 1TB limit
      const state = tracker.getState();

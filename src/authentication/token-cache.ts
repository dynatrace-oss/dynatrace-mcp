...
/** Directory where file-based token caches are stored */
const TOKEN_CACHE_DIR = path.join(os.homedir(), '.config', 'dynatrace-mcp');
...
/** Sanitize an account name so it is safe to use as a filename component */
function sanitizeForFilename(account: string): string {
  return account.replace(/[^a-zA-Z0-9_-]/g, '_');
}

/**
 * File-based token cache that persists OAuth tokens to ~/.config/dynatrace-mcp/token-<account>.json.
 * Only active when DT_MCP_TOKEN_STORAGE=file is set.
...
export class FileTokenCache implements TokenCache {
  ...
  constructor(private readonly account: string) {
    this.filePath = path.join(TOKEN_CACHE_DIR, `token-${sanitizeForFilename(account)}.json`);
  }

  /**
   * Loads a previously stored token from the cache file (if it exists).
   * Must be called once before using the cache.
   */
  async initialize(): Promise<void> {
    try {
      ...
      if (code !== 'ENOENT') {
        console.error(`⚠️ Failed to load token from file cache: ${e instanceof Error ? e.message : String(e)}`);
      }
    }
  }
...
// Module-level map of per-account FileTokenCache instances (keyed by clientId).
const fileCacheInitPromises = new Map<string, Promise<FileTokenCache>>();

/**
 * Returns (and lazily initializes) a FileTokenCache for the given account name.
 * Subsequent calls with the same account return the same initialized instance.
 */
export function getOrCreateFileCache(account: string): Promise<FileTokenCache> {
  if (!fileCacheInitPromises.has(account)) {
    const initPromise = (async () => {
      const cache = new FileTokenCache(account);
      await cache.initialize();
      return cache;
    })();
    fileCacheInitPromises.set(account, initPromise);
  }
  return fileCacheInitPromises.get(account)!;
}

/**
 * Returns the appropriate token cache based on DT_MCP_TOKEN_STORAGE env var.
 * - 'file': file-based cache (~/.config/dynatrace-mcp/token-<account>.json)
 * - default: OS keychain via @napi-rs/keyring
 */
export function getOrCreateTokenCache(account: string): Promise<KeychainTokenCache | FileTokenCache> {
  if (process.env.DT_MCP_TOKEN_STORAGE === 'file') {
    return getOrCreateFileCache(account);
  }

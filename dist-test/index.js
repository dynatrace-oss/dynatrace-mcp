...
const main = async () => {
  ...
  const createAuthenticatedHttpClient = async (scopes) => {
    ...
    return await (0, dynatrace_clients_1.createDtHttpClient)(
      dtEnvironment,
  ...
  try {
    ...
    const environmentInformationClient = new client_platform_management_service_1.EnvironmentInformationClient(
      dtClient,
    );
    ...
    if ((0, shared_errors_1.isClientRequestError)(error)) {
      console.error(
        ...
        (0, dynatrace_connection_utils_1.handleClientRequestError)(error),
      );
    } else {
      console.error(`❌ Failed to connect to Dynatrace environment ${dtEnvironment}:`, error.message);
  ...
  const createConfiguredMcpServer = () => {
    ...
    const wrapToolCallback = (name, cb) => {
      return async (args, _extra) => {
        ...
        const rateLimitResult = (0, rate_limiter_1.getToolCallRateLimiter)().check(startTime);
    ...
    tool(
      ...
      async ({ timeframe, riskScore, additionalFilter, maxVulnerabilitiesToDisplay }) => {
        const dtClient = await createAuthenticatedHttpClient(
          scopesBase.concat('storage:events:read', 'storage:buckets:read', 'storage:security.events:read'),
        );
        const result = await (0, list_vulnerabilities_1.listVulnerabilities)(
          dtClient,
          additionalFilter,
          riskScore,
          timeframe,
    ...
    tool(
      ...
      async ({ timeframe, status, additionalFilter, maxProblemsToDisplay }) => {
        ...
        const result = await (0, list_problems_1.listProblems)(dtClient, additionalFilter, status, timeframe);
    ...
    tool(
      ...
      async ({ entityNames, maxEntitiesToDisplay, extendedSearch }) => {
        ...
        const smartscapeResult = await (0, find_monitored_entity_by_name_1.findMonitoredEntityViaSmartscapeByName)(
          dtClient,
          entityNames,
        );
        if (smartscapeResult && smartscapeResult.records && smartscapeResult.records.length > 0) {
          ...
          const validSmartscapeEntities = smartscapeResult.records.filter(
            (entity) => !!(entity && entity.id && entity.type && entity.name),
          );
        ...
        const result = await (0, find_monitored_entity_by_name_1.findMonitoredEntitiesByName)(
          dtClient,
          entityNames,
          extendedSearch,
        );
        if (result && result.records && result.records.length > 0) {
          ...
          const validClassicEntities = result.records.filter(
            (entity) => !!(entity && entity.id && entity['entity.type'] && entity['entity.name']),
          );
    ...
    tool(
      ...
      async ({ channel, message }) => {
        ...
        const response = await (0, send_slack_message_1.sendSlackMessage)(
          dtClient,
          slackConnectionId,
          channel,
          message,
    ...
    registerAppTool(
      ...
      wrapToolCallback('execute_dql', async ({ dqlStatement, recordLimit = 100, recordSizeLimitMB = 1 }) => {
        ...
        const response = await (0, execute_dql_1.executeDql)(
          dtClient,
    ...
    registerAppResource(server, 'DQL Results Viewer', executeDqlResourceUri, {}, async () => {
      const html = (0, node_fs_1.readFileSync)(
        (0, node_path_1.join)(__dirname, 'ui', 'execute-dql', 'index.html'),
        'utf-8',
      );
      return {
    ...
    tool(
      ...
      async ({ problemType, teamName, channel, isPrivate }) => {
        ...
        const response = await (0, create_workflow_for_problem_notification_1.createWorkflowForProblemNotification)(
          dtClient,
          teamName,
          channel,
          problemType,
    ...
    tool(
      ...
      async ({ timeframe, clusterId, kubernetesEntityId, eventType, maxEventsToDisplay }) => {
        ...
        const result = await (0, get_events_for_cluster_1.getEventsForCluster)(
          dtClient,
          clusterId,
          kubernetesEntityId,
          eventType,
...
`;
    ...
    tool(
      ...
      'Send a custom event to Dynatrace using the Events API v2. Use this to ingest custom events for alerting, tracking deployments, configuration changes, or any custom business events.',
      {
        ...
        title: zod_1.z.string().describe('Title of the event (max 500 characters). Should be descriptive and concise.'),
        entitySelector: zod_1.z
          .string()
          .optional()
          .describe(
      ...
      async ({ eventType, title, entitySelector, properties, startTime, endTime }) => {
        ...
        const result = await (0, send_event_1.sendEvent)(dtClient, {
          eventType: eventType,
          title,
          entitySelector,
          properties,
    ...
    tool(
      ...
      'List all exceptions known on Dynatrace starting with the most recent.',
      {
        ...
        maxExceptionsToDisplay: zod_1.z
          .number()
          .default(10)
      ...
      async ({ timeframe, additionalFilter, maxExceptionsToDisplay }) => {
        ...
        const result = await (0, list_exceptions_1.listExceptions)(
          dtClient,
          additionalFilter,
          timeframe,
    ...
    tool(
      ...
      async ({}) => {
        ...
        const analyzers = await (0, davis_analyzers_1.listDavisAnalyzers)(dtClient);
    ...
    tool(
      ...
      {
        analyzerName: zod_1.z
          .string()
          .describe('The name of the Davis Analyzer to execute (e.g., "dt.statistics.GenericForecastAnalyzer")'),
        input: zod_1.z
          .record(zod_1.z.string(), zod_1.z.any())
          .optional()
          .describe('Input parameters for the analyzer as a JSON object'),
        timeframeStart: zod_1.z
          .string()
          .optional()
          .default('now-1h')
          .describe('Start time for the analysis (default: now-1h)'),
        timeframeEnd: zod_1.z.string().optional().default('now').describe('End time for the analysis (default: now)'),
      },
      {
        readOnlyHint: true,
      },
      async ({ analyzerName, input = {}, timeframeStart = 'now-1h', timeframeEnd = 'now' }) => {
        ...
        try {
          // Execute Davis Analyzer
          const result = await (0, davis_analyzers_1.executeDavisAnalyzer)(dtClient, analyzerName, {
            generalParameters: {
              timeframe: {
    ...
    tool(
      ...
      'Create a new notebook in the Dynatrace platform (NOT a Jupyter notebook) to share your analysis and findings with colleagues.',
      {
        ...
        content: zod_1.z
          .array(
            zod_1.z.object({
              type: zod_1.z.enum(['dql', 'markdown']),
              text: zod_1.z.string(),
            }),
          )
          .describe(
      ...
      async ({ name, content, description }) => {
        ...
        const data = await (0, notebooks_1.createDynatraceNotebook)(dtClient, name, content, description);
        return data

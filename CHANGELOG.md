...
- Added automated UI screenshots for MCP App pull requests: a new GitHub Actions workflow captures Playwright screenshots of the `execute_dql` MCP App UI in all key states (table view, loading, error, empty results) and uploads them as workflow artifacts, commenting on the PR with a download link.
